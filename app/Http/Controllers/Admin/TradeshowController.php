...
class TradeshowController extends Controller
{
    ...
    public function store(ValidationController $request)
    {
        ...
        return redirect::back()->with(compact('data'))->with('flash_message', 'Trade Added Successfully');
...
//        exit;
        ...
        // return Redirect::route('admin.tradeshow-show')->with('flash-message','Updated Successfully !!');
        return redirect::back()->with('flash_message', 'Trade Updated Successfully');
    ...
    public function delete($id)
    {
        ...
        return Redirect::back()->with('flash_message', 'TradeShow Information deleted');;
    }
       ...
       $result= '';
        if($request->title){
            $result = DB::table('bdtdc_tradeshows as bt')
                    ...
                    ($request->title) ? $result->where('btd.title', 'like', '%' . $request->title . '%') : null;
        }elseif($request->venue){
            $result = DB::table('bdtdc_tradeshows as bt')
                    ->join('bdtdc_tradeshow_descriptions as btd','btd.tradeshow_id','=','bt.id')
                    ->join('bdtdc_country as c','c.id','=','bt.country_id');
                    ($request->venue) ? $result->where('bt.venue', 'like', '%' . $request->venue . '%') : null;
        }elseif($request->country_id){
            $result = DB::table('bdtdc_tradeshows as bt')
                    ->join('bdtdc_tradeshow_descriptions as btd','btd.tradeshow_id','=','bt.id')
                    ->join('bdtdc_country as c','c.id','=','bt.country_id');
                    ($request->country_id) ? $result->where('bt.country_id',$request->country_id) : null;
        }
        if($result){       
            $result = $result->get(['btd.title','bt.id','bt.venue','c.name as country','btd.created_at']);
            $country = Country::get(['name','id']);
        }else{
            return Redirect::back()->with('error_message', 'Please Enter Trade Searching Details');
        }

        // return $result;
        return view('protected.admin.trade.trade_search',compact(['result','country']));
